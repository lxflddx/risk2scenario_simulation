...
# 继承Statement基类，实现ConstructorStatement类，实现clone，update_ast_node，mutate方法
class ConstructorStatement(Statement):
    ...
    def __init__(self, testcase, assignee: str, constructor_name: str, args: dict, arg_bounds: dict):
        ...
        self._args = args
        # self._arg_bounds = self._infer_bounds_from_args()
        self._arg_bounds = arg_bounds
        self._default_arg_bounds = None
        self._ast_node = None
    ...
    def _infer_bounds_from_args(self):
        bounds = {}
        for key, value in self._args.items():
            if key in ["offset", "initial_speed"]:
                if isinstance(value, (int, float)):
                    delta = abs(value) * 0.2
                    bounds[key] = [value - delta, value + delta]
                else:
    ...
    #     深拷贝当前语句。
    #     """
    #     new_args = {k: v for k, v in self._args.items()}
    #     return ConstructorStatement(self._testcase, self._assignee, self._constructor_name, new_args)
    ...
    #
    #     for key, value in self._args.items():
    #         if key in ["initial_speed", "offset"]:
    #             var.append(value)
    #             lb.append(self._arg_bounds[key][0])
    #             ub.append(self._arg_bounds[key][1])
    #             keys.append(key)
    #     if var:  # 如果有需要变异的变量
    #         mutated_var = polynomial_mutate(var, lb, ub)
    #
    #     for arg_name, arg_value in self._args.items():
    #         # 这里是随机指定的车道id
    #         if arg_name == 'lane_id':
    #             self._args[arg_name] = random.choice([0, 1, 2, 3])
    #         else:
    #             self._args[arg_name] = mutated_var.pop(0)
...
# 继承Statement基类，实现MethodStatement类，实现clone，update_ast_node，mutate方法
class MethodStatement(Statement):
    ...
    def clone(self, test_case):
        ...
        return MethodStatement(test_case, copy.deepcopy(self.callee), copy.deepcopy(self.method_name), clone_args,
                               clone_arg_bounds)
    ...
    def mutate(self):
        ...
        lb = []
        ub = []
        logger.info("arg bounds: %s", str(self.arg_bounds))
        for arg_name, arg_bound in self.arg_bounds.items():
            if arg_name == "target_speed":
                var.append(self.args[arg_name])
                lb.append(arg_bound[0])
                ub.append(arg_bound[1])
        ...
        for arg_name, arg_bound in self.arg_bounds.items():
            if arg_name == "target_speed":
                self.args[arg_name] = mut_var.pop(0)
            elif arg_name == "time_interval":
                new_time_interval = random.randint(int(arg_bound[0]), int(arg_bound[1]))
                self.args[arg_name] = new_time_interval
            else:
                self.args[arg_name] = random.choice(arg_bound)
